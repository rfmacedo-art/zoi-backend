...
class EnhancedRiskCalculator:
    """
    Calculadora de risco melhorada com análise baseada em NCM
    e componentes mapeados para visualização no frontend
    """
    
    def calculate(self, product, rasff_alerts_6m=0, rasff_alerts_12m=0):
        ncm = product.ncm_code
        profile = NCM_RISK_PROFILES.get(ncm, None)
        
        if profile:
            print(f"📊 Usando perfil de risco NCM para {profile['name']}")
            ...
            rasff_penalty = min(rasff_alerts_12m * 3, 15)
            sanitario -= rasff_penalty
            fitossanitario -= rasff_penalty * 0.5
            
            if profile['eu_barriers'] == 'high':
                fitossanitario -= 10
                documental -= 8
            elif profile['eu_barriers'] == 'medium':
                fitossanitario -= 5
                documental -= 4
            ...
            print(f"⚠️  Perfil NCM não encontrado, usando cálculo genérico")
            sanitario = 85.0
            fitossanitario = 80.0
            logistico = 88.0
            documental = 82.0
            
            rasff_penalty = min(rasff_alerts_12m * 5, 25)
            sanitario -= rasff_penalty
            fitossanitario -= rasff_penalty * 0.7
        
        sanitario = max(0, min(100, sanitario))
        fitossanitario = max(0, min(100, fitossanitario))
        logistico = max(0, min(100, logistico))
        documental = max(0, min(100, documental))
        ...
        recommendations = self._generate_recommendations(
            product, profile, sanitario, fitossanitario, 
            logistico, documental
        )
        ...
        return {
            ...
            "alerts": {
                "rasff_6m": rasff_alerts_6m,
                "rasff_12m": rasff_alerts_12m,
                "historical_rejections": profile['historical_rejections'] if profile else 0
            }
        }
    
    def _generate_recommendations(self, product, profile, sanitario, 
                                 fitossanitario, logistico, documental):
        recs = []
        
        if sanitario < 70:
            recs.append("⚠️ Risco sanitário elevado - Reforçar análises laboratoriais de LMR")
        
        if fitossanitario < 70:
            recs.append("🌱
            recs.append("🌱 Atenção às barreiras fitossanitárias - Certificação obrigatória")
        
        if logistico < 75:
            recs.append("📦 Revisar condições de transporte e temperatura")
        
        if documental < 75:
            recs.append("📄 Documentação incompleta - Verificar certificados de origem")
        
        if profile:
            if profile['eu_barriers'] == 'high':
                recs.append(f"🇪🇺 Barreiras UE elevadas para {profile['name']} - Consultoria especializada recomendada")
            
            if profile['common_issues']:
                issues_str = ", ".join(profile['common_issues'][:2])
                recs.append(f"🔍 Pontos críticos históricos: {issues_str}")
        
        if product.direction.value == "export":
            recs.append("📤 Exportação BR→IT - Verificar compliance RASFF")
        else:
            recs.append("📥 Importação IT→BR - Validar certificações europeias")
        
        if not recs:
            recs.append("✅ Produto em conformidade - Manter padrões de qualidade")
...
def export_risk_pdf(product_key: str, db: SessionLocal = Depends(get_db)):
    """
    Endpoint Premium: Exporta relatório executivo de risco em PDF
    """
    print(f"📄 Gerando PDF para produto: {product_key}")
...
<body>
    ...
    <div class="section">
        ...
        <div class="info-grid">
            ...
            <div class="info-item">
                ...
                <div class="info-value">{'Exportação BR→IT' if product.direction.value == 'export' else 'Importação IT→BR'}</div>
            </div>
            <div class="info-item">
    ...
    <div class="section">
        ...
        <div class="alert-box">
            ...
            <p>📊 Rejeições históricas (NCM): <strong>{assessment_data['alerts']['historical_rejections']} casos</strong></p>
        </div>
    </div>
    ''' if assessment_data['alerts']['rasff_12m'] > 0 or assessment_data['alerts']['historical_rejections'] > 0 else ''}
    ...
    <div class="section">
        ...
        <div class="info-grid">
            <div class="info-item">
                ...
                <div class="info-value">{profile['eu_barriers'].upper()}</div>
            </div>
            <div class="info-item">
                <div class="info-label">Problemas Comuns</div>
                <div class="info-value" style="font-size: 14px;">{', '.join(profile['common_issues'][:2])}</div>
            </div>
        </div>
    </div>
    ''' if profile else ''}
    ...
    <div class="section">
        ...
        <div class="recommendations">
            {"".join([f'<div class="recommendation-item">{rec}</div>' for rec in assessment_data['recommendations']])}
        </div>
    </div>
